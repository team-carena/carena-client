...
const CRITERIA_LIST: CriteriaItem[] = [
	{
		label: "정상A",
		descriptions: ["건강이 양호", "모든 검진 항목이 정상 범위에 해당하는 경우"],
	},
	{
		label: "정상B",
		variant: "borderline",
			...
			<div className="flex flex-col gap-[2rem]">
				...
				{CRITERIA_LIST.map((item) => (
					<div key={item.label} className="flex flex-col gap-[0.8rem]">
						<LargeBadge variant={item.variant} className="w-fit">
							{item.label}
						</LargeBadge>
						...
						<ul className="flex flex-col gap-[0.2rem]">
							{item.descriptions.map((desc) => (
								<li
									key={desc}
									className="ml-[0.5rem] flex items-start gap-[1rem]"
								>
									<span className="mt-[1rem] h-[0.3rem] w-[0.3rem] shrink-0 rounded-full bg-gray-900" />
									<span>{desc}</span>
								</li>
							))}
						</ul>
