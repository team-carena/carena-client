...
const Bullet = () => (
	<span className="h-[0.25rem] w-[0.25rem] shrink-0 rounded-full bg-gray-900" />
);
...
export const MenuDetailPage = () => {
	...
	return (
		<div className="flex h-dvh flex-col overflow-hidden bg-gray-50">
			<main className="flex-1 overflow-y-auto pt-[2.8rem] pb-[2rem]">
				{/* 메뉴 타이틀 및 설명 */}
				<section
					aria-labelledby="menu-detail-title"
					className={`${SECTION_LAYOUT} gap-[2rem] px-[2rem]`}
				>
				...
				<section
					...
					className={`${SECTION_LAYOUT} gap-[1.2rem] px-[1.6rem] pt-[4rem]`}
				>
				...
				>
					...
					<p
						id="caution-title"
						className="head03-sb-16 px-[2rem] text-gray-900"
					>
						주의 식품
					</p>
					...
					<div className="w-full px-[1.6rem]">
						<ContentCard variant="muted" className="w-full">
							<ContentCard.Content>
								<ul className="flex flex-col gap-[0.4rem]">
									{(data?.cautionary ?? []).map((food) => (
										<li key={food} className="flex items-center gap-[0.8rem]">
											<Bullet />
					...
					{/* 출처 */}
					<p className="body06-r-10 h-[1.5rem] self-end px-[2rem] text-gray-700">
						출처: 국내 의료기관 영양지침
					</p>
				</section>
			</main>
